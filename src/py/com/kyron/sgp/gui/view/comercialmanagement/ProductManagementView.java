...
public class ProductManagementView extends VerticalLayout implements View {

	private final Logger logger = LoggerFactory.getLogger(ProductManagementView.class);
	private Map<String,String> messages;
	private final String VIEW_NAME = "product.management.";
	...
	public ProductManagementView(Component... children) {
		super(children);
		// TODO Auto-generated constructor stub
	}
    ...
    public void detach() {
	 ...
	 private void buildProductTable() throws PmsServiceException{
	    	...
	    	this.productTable.setColumnExpandRatio("operations", 0.019f); 
	 ...
	 private void editProduct(ProductDTO vProductDTO,  boolean massiveInsertMode){
		try{
			UI.getCurrent().getNavigator().navigateTo(DashboardViewType.REGISTER_PRODUCT_FORM.getViewName());
			DashboardEventBus.post(new ProductRegisterFormViewEvent(vProductDTO, DashboardViewType.PRODUCT_MANAGEMENT.getViewName(), massiveInsertMode));
		}catch(Exception e){
			logger.error("\nerror: ",e);
		}
	 ...
	 private Component buildToolbar() {
	        ...
	        HorizontalLayout tools = new HorizontalLayout(this.buildFilter(),this.createRegisterProductButton());	        	        
	 ...
	 private Button createRegisterProductButton(){
	        final Button registerMachine = new Button(this.messages.get(this.VIEW_NAME + "button.register.product"));
	        registerMachine.setDescription(this.messages.get(this.VIEW_NAME + "button.register.product.description"));
	        registerMachine.addClickListener(new ClickListener() {
	            @Override
	            public void buttonClick(final ClickEvent event) {
	            	editProduct(null, true);
	            }
	        });
	...
	private HorizontalLayout buildOperationsButtonPanel(final ProductDTO vProductDTO){
		final Button has_no_valid_production_processIndicatorButton = new Button();
		has_no_valid_production_processIndicatorButton.addStyleName("borderless");
		has_no_valid_production_processIndicatorButton.setIcon(FontAwesome.EXCLAMATION_CIRCLE);		
		has_no_valid_production_processIndicatorButton.setVisible(!vProductDTO.getHas_production_process());
		has_no_valid_production_processIndicatorButton.setDescription(messages.get(VIEW_NAME + "table.product.column.operations.button.has.no.valid.production.process.description"));
		has_no_valid_production_processIndicatorButton.setResponsive(false);
		
		final Button editButton = new Button();
		editButton.setIcon(FontAwesome.EDIT);
		editButton.setDescription(this.messages.get("application.common.table.column.operations.edit"));
		...
		editButton.addClickListener(new ClickListener() {
            ...
            public void buttonClick(final ClickEvent event) {
                ...
                try{		                	
                	editProduct(vProductDTO, false);
                }catch(Exception e){
                	commonExceptionErrorNotification.showErrorMessageNotification(e);
                }			                
            }
        });
		HorizontalLayout vHorizontalLayout = new HorizontalLayout(editButton,has_no_valid_production_processIndicatorButton);
