/*69-p_calculate_pms_order_item_raw_material_suff_report_detail-201611031956*/
	...
	begin
		...
		begin
			...
			for vorder_prod_raw_mat_measur_unit_req_for_one_unit_record in vorder_prod_raw_mat_measur_unit_req_for_one_unit_cursor loop
				...
				vtotal_quantity_per_raw_material_and_measurment_unit := trunc((vorder_prod_raw_mat_measur_unit_req_for_one_unit_record.required_one_unit_quantity * vunsatisfiable_quantity),2);
				...
				v_missing_amount := vraw_material_measurment_unit_calculated_qty -  trunc((vorder_prod_raw_mat_measur_unit_req_for_one_unit_record.required_one_unit_quantity * pminimal_quantity_capable_of_produce),2) - 
trunc((vunsatisfiable_quantity * vorder_prod_raw_mat_measur_unit_req_for_one_unit_record.required_one_unit_quantity),2);
				...
				if v_missing_amount < 0 then
					...
					sending the 
					RAW MATERIAL AND MEASURMENT UNIT REQUIRED QUANTITY 
					TO SATISFY THE
					UNSATISFIABLE PRODUCT QUANTITY
					THE DIFERENCE: 
					THIS IS THE RAW MATERIAL AND MEASURMENT UNIT
					MISSING QUANTITY OR THE QUANTITY NEEDED TO BE PURCHASED
					IN ORDER TO INSTANTIATE THE UNSATISFIABLE PRODUCT QUANTITY
					CAN BE CALCULATED BY the diference between
					pms_order_item_raw_material_sufficiency_report_detail.required_quantity
					-
					pms_order_item_raw_material_sufficiency_report_detail.available_quantity
					...
					*/
					...
					PERFORM p_i_pms_order_item_raw_material_sufficiency_report_detail
						...
						(vraw_material_measurment_unit_calculated_qty -  trunc((vorder_prod_raw_mat_measur_unit_req_for_one_unit_record.required_one_unit_quantity * pminimal_quantity_capable_of_produce),2)),--prequired_quantity
