...
create table pms_manpower_cost(
id	   		 		   bigint,
...
ALTER TABLE pms_manpower_cost ADD CONSTRAINT pms_manpower_cost_id_pk PRIMARY KEY (id);

--ALTER TABLE pms_manpower_cost ADD is_active varchar(1) default 'S'
ALTER TABLE pms_manpower_cost ADD CHECK (is_active in ('S'));

ALTER TABLE pms_manpower_cost ADD CONSTRAINT
pms_manpower_cost_uk_01 UNIQUE 
(is_active);

ALTER TABLE pms_manpower_cost 
ADD constraint pms_manpower_cost_fk_01
FOREIGN KEY (id_tariff) REFERENCES pms_tariff (id);


create sequence pms_manpower_cost_id_sq
increment by 1
minvalue 1
no maxvalue
start with 1
no cycle
owned by pms_manpower_cost.id;


--drop function 
create or replace function p_i_pms_manpower_cost
(
pid	   		 		   bigint,
pid_tariff	   		 	bigint,
ptariff_amount	   		 		   numeric,
pregistration_date			timestamp,
...
begin
	RAISE INFO '--------------------------';
	RAISE INFO 'executing p_i_pms_manpower_cost';
	RAISE INFO '--------------------------';
	...
	/*BEGIN VALIDATIONS*/	
		...
		then
			...
			begin
				b1_error_message:='py.com.kyron.sgp.persistence.productionmanagement.dao.manpowercostdao.manpowercostdto.tariffamount.required.error'||'end.of.message';
	...
	insert into pms_manpower_cost
			(id,id_tariff,tariff_amount,registration_date,creation_user,creation_date)
	values
			(pid,
			pid_tariff,
			ptariff_amount,
...
create or replace function p_u_pms_manpower_cost
(
pid	   		 		   bigint,
pvalidity_end_date			timestamp,
...
begin
	RAISE INFO '--------------------------';
	RAISE INFO 'executing p_u_pms_manpower_cost';
	RAISE INFO '--------------------------';
