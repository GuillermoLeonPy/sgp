/*34-UC-REG-PED-f_calculate_activity_machine_use_cost-201609291444*/
create or replace function f_calculate_activity_machine_use_cost
(
...
begin
	RAISE INFO '--------------------------';
	RAISE INFO 'executing f_calculate_activity_machine_use_cost';
	RAISE INFO '--------------------------';
	...
	declare
		...
		mr.id_machine,mr.minutes_quantity
	...
	begin
		for vlooprecord in vcursor_machine_requeriment loop
			select tariff.tariff_id,tariff.id_currency,mc.tariff_amount into vrecord
...
select * from v_activities_machine_requirement_cost
15 * 0.18 = 2,7 ussd --> gs 2.7 * 5550 = 14985
select f_calculate_activity_machine_use_cost(7,61);


create view v_activities_machine_requirement_cost as
select 
ppa.activity_id,
m.machine_id,
mr.minutes_quantity,
...
pms_machine_requirement mr
	on mr.id_production_process_activity = ppa.id 
join 
pms_machine m
	on m.id = mr.id_machine
left outer join 
pms_machine_use_cost mc
	on mr.id_machine = mc.id_machine and mc.validity_end_date is null and mc.is_active = 'S'
left outer join 
pms_tariff tariff
	on mc.id_tariff = tariff.id
where
	--ppa.id = 7
	--and 
