/*63-f_calculate_activity_man_power_cost_identifiying_order-201610611837*/

create or replace function f_calculate_activity_man_power_cost_identifiying_order
(
...
begin
	RAISE INFO '--------------------------';
	RAISE INFO 'executing f_calculate_activity_man_power_cost_identifiying_order';
	RAISE INFO '--------------------------';
	...
	declare
		...
		select mpr.minutes_quantity, mpr.id id_man_power_requirement
	...
	begin
		for vlooprecord in vcursor_man_power_requirement loop
			select tariff.tariff_id,tariff.id_currency,mpc.tariff_amount,
			mpc.id id_manpower_cost into vrecord
			...
			if pid_order_budget_production_process_activity is not null then
				PERFORM p_i_pms_order_budget_production_process_activity_manpower(pid_order_budget_production_process_activity,vlooprecord.id_man_power_requirement,vrecord.id_manpower_cost,pcreation_user);
			end if;
