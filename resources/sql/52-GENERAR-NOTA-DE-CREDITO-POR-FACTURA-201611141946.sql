/*52-GENERAR-NOTA-DE-CREDITO-POR-FACTURA-201611141946*/
...
begin
	...
	declare
		...
		vquantity_available_to_credit_note		pms_credit_note_item.quantity%type;
	begin
		...
		/*validate the pquantity: */
		vquantity_available_to_credit_note := vrecord.quantity - vrecord.product_deliver_blocked_by_doc_quantity;
		if pquantity > vquantity_available_to_credit_note then
			declare
				b1_error_message		text;
				b1_error_message_hint	text;
			begin
				b1_error_message:='py.com.kyron.sgp.persistence.cash.movements.management.dao.generate.credit.note.item.by.sale.invoice.item.requested.quantity.greater.than.available.to.credit.note.quantity.error'||''||pquantity||''||vquantity_available_to_credit_note||''||v_product_id||'end.of.message';
				b1_error_message_hint:=b1_error_message;
				RAISE EXCEPTION '%',b1_error_message USING HINT = b1_error_message_hint, ERRCODE ='P9989';
			end;
		end if;
		...
		/* */
		select * into vorder_item_record from pms_order_item where id = vrecord.id_order_item;

		if vorder_item_record.pending_to_instanciate_quantity > 0  
		and ((vorder_item_record.pending_to_instanciate_quantity - pquantity) > 0)then
			update pms_order_item
			set pending_to_instanciate_quantity = vorder_item_record.pending_to_instanciate_quantity - pquantity,
			canceled_entering_production_by_document_quantity = vorder_item_record.canceled_entering_production_by_document_quantity + pquantity,
			last_modif_user = pcreation_user, last_modif_date = now()
			where id = vorder_item_record.id;
		elsif vorder_item_record.pending_to_instanciate_quantity > 0 then
		--in this case vorder_item_record.pending_to_instanciate_quantity - pquantity <= 0
			update pms_order_item
			set pending_to_instanciate_quantity = 0,
			canceled_entering_production_by_document_quantity = vorder_item_record.canceled_entering_production_by_document_quantity + vorder_item_record.pending_to_instanciate_quantity,
			last_modif_user = pcreation_user, last_modif_date = now()
			where id = vorder_item_record.id;			
		end if;

		update pms_sale_invoice_item
		set product_deliver_blocked_by_doc_quantity = vrecord.product_deliver_blocked_by_doc_quantity + pquantity,
		last_modif_user = pcreation_user, last_modif_date = now()
		where id = vrecord.id;
	end;	
