...
--DROP FUNCTION p_i_pms_person(bigint, character varying, character varying, character varying, character varying, character varying, character varying, bigint, character varying, character varying, character varying, character varying, character varying);
...
pis_functionary		 		   varchar,
pis_customer		 		   varchar,
pis_supplier		 		   varchar,
pcreation_user	 		   varchar,
papp_user_name				varchar,
papp_user_passwd			varchar
)
returns void as
$BODY$
declare
...
begin
	...
	if (((papp_user_name is not null and length(trim(papp_user_name))>0) and 
		(papp_user_passwd is null or length(trim(papp_user_passwd))=0))
		or
		((papp_user_passwd is not null and length(trim(papp_user_passwd))>0) and 
		(papp_user_name is null or length(trim(papp_user_name))=0))
		)
	then
		declare
			b1_error_message		varchar(200);
			b1_error_message_hint	varchar(200);
		begin
			b1_error_message:='py.com.kyron.sgp.persistence.personsmanagement.dao.persondao.insert.appusername.apppassword.required.error'||'end.of.message';
			b1_error_message_hint:=b1_error_message;
			RAISE EXCEPTION '%',b1_error_message USING HINT = b1_error_message_hint, ERRCODE ='P9984';
		end;
	end if;

	if papp_user_name is not null and length(trim(papp_user_name))>0 then
		declare
			vid					bigint;
			b1_error_message		varchar(200);
			b1_error_message_hint	varchar(200);
		begin
			select id into vid from pms_person where app_user_name = lower(trim(papp_user_name));
			if vid is not null then
				b1_error_message:='py.com.kyron.sgp.persistence.personsmanagement.dao.persondao.insert.appusername.already.exists.error'||''||lower(trim(papp_user_name))||'end.of.message';
				b1_error_message_hint:=b1_error_message;
				RAISE EXCEPTION '%',b1_error_message USING HINT = b1_error_message_hint, ERRCODE ='P9983';
			end if;
		end;
	end if;
	/*END OF VALIDATIONS*/
	...
	insert into pms_person
			...
			personal_telephone_number,personal_civil_id_document,commercial_name,is_functionary,
			is_customer,is_supplier,creation_user,creation_date,app_user_name,app_user_passwd)
	values
			(pid,
			upper(trim(pruc)),
			...
			pcreation_user,
			now(),
			lower(trim(papp_user_name)),
			lower(trim(papp_user_passwd)));
	
	EXCEPTION
		WHEN SQLSTATE 'P9989' or SQLSTATE 'P9988' or SQLSTATE 'P9987' or SQLSTATE 'P9986' or SQLSTATE 'P9985' or SQLSTATE 'P9984' or SQLSTATE 'P9983' THEN
