/*61-f_calculate_activity_raw_materials_cost_identifiying_order-201610311756*/

create or replace function f_calculate_activity_raw_materials_cost_identifiying_order
(
...
begin
	RAISE INFO '--------------------------';
	RAISE INFO 'executing f_calculate_activity_raw_materials_cost_identifiying_order';
	RAISE INFO '--------------------------';
	...
	begin
		...
		where 
			...
			group by 
			prod.product_id,pp.process_id,pp.id,ppa.activity_id;
	...
	declare
		...
		rmr.id_raw_material,rmr.id_measurment_unit,rmr.quantity,rmr.id id_raw_material_requirement
	...
	begin
		for vlooprecord in vcursor_raw_material_requeriments loop
			select tariff.tariff_id,tariff.id_currency,rmc.tariff_amount, 
			rmc.id id_raw_material_cost into vrecord
			...
			if pid_order_budget_production_process_activity is not null then
				PERFORM p_i_pms_order_budget_production_process_activity_raw_m(pid_order_budget_production_process_activity,vlooprecord.id_raw_material_requirement,vrecord.id_raw_material_cost,pcreation_user);
			end if;
